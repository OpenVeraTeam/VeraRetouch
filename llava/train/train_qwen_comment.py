...
"""
根据给定参数和条件，处理参数以适应DeepSpeed的Zero优化

Args:
 param (torch.Tensor): 需要处理的参数
 ignore_status (bool, optional): 是否忽略参数状态. 默认为 False
...
"""
获取PEFT状态，根据bias参数选择性地包含bias参数

Args:
 named_params (dict): 包含参数名称和张量的字典
 bias (str): 指定是否包含bias参数，可选值为 "none", "all", "lora_only"

Returns:
 dict: 包含选定参数的字典，其中值可能经过 `maybe_zero_3` 处理
"""
...
Args:
 named_params (dict): 包含参数名称和张量的字典
 require_grad_only (bool, optional): 如果为True，只返回需要梯度的参数，默认为True

Returns:
 dict: 包含处理后的参数名称和张量的字典
"""
...
Args:
 named_params (dict): 包含参数名和张量的字典
 keys_to_match (list): 需要匹配的键列表

Returns:
 dict: 匹配键的参数状态，值可能为零，并已移至CPU
"""
...
Args:
 model (torch.nn.Module): 需要查找的模型

Returns:
 list: 包含模型中所有线性层名字的列表
"""
...
"""
安全保存Hugging Face Trainer的模型到磁盘

Args:
 trainer (transformers.Trainer): Hugging Face Trainer实例
...
Args:
 special_tokens_dict (Dict): 特殊标记的字典
 tokenizer (transformers.PreTrainedTokenizer): 分词器对象
 model (transformers.PreTrainedModel): 模型对象
...
Returns:
 Dict: 包含分词后的输入ID、标签、输入ID长度和标签长度的字典
"""
...
Args:
 target (list): 目标数组，将被修改
 tokenized_lens (list of int): 每个部分的长度
 speakers (list of str): 每个部分的说话者标识

Returns:
 None: 该方法不返回任何值，直接修改传入的 target 数组
"""
...
Args:
 header (str): 对话的初始头部信息
 source (list): 包含对话轮次的列表，每个元素是一个字典，包含 'from' 和 'value' 键
 get_conversation (bool, optional): 是否返回完整的对话内容，默认为 True
...
Args:
 sources (list): 包含对话数据的列表，每个元素是一个包含消息字典的列表
 tokenizer (transformers.PreTrainedTokenizer): 用于分词的预训练分词器
 has_image (bool, optional): 是否包含图像数据，默认为False

Returns:
 dict: 包含分词后的输入ID和标签的字典
"""
...
Args:
 sources (List[Dict]): 包含对话数据的列表，每个元素是一个字典，字典中包含对话的来源和内容。
 tokenizer (transformers.PreTrainedTokenizer): 用于将文本转换为token ID的分词器。
 has_image (bool, optional): 是否包含图像数据。默认为False。

Returns:
 Dict: 包含预处理后的输入ID和标签的字典。
"""
def preprocess_qwen_2(
    sources,
    tokenizer: transformers.PreTrainedTokenizer,
...
"""
预处理版本1，用于处理对话数据并将其转换为模型输入

Args:
 sources (list): 包含对话数据的列表，每个对话是一个字典列表，字典包含 "from" 和 "value" 键
 tokenizer (transformers.PreTrainedTokenizer): 用于将文本转换为模型输入的分词器
 has_image (bool, optional): 是否包含图像数据，默认为 False

Returns:
 Dict: 包含 'input_ids' 和 'labels' 的字典，分别表示模型的输入和标签
"""
def preprocess_v1(
    sources,
    tokenizer: transformers.PreTrainedTokenizer,
...
Args:
 sources (Sequence[str]): 包含文本数据的序列，每个元素是一个包含两个字典的列表，其中第一个字典的值包含默认图像标记。
 tokenizer (transformers.PreTrainedTokenizer): 用于对文本进行分词的预训练分词器。
...
"""
预处理给定的对话列表，每个对话列表包含多个句子。此函数执行以下操作：
1. 在每个句子的开头添加信号 '### '，并在末尾添加信号 '\n'；
2. 将对话连接在一起；
3. 对连接后的对话进行分词；
4. 深拷贝作为目标，并将人类的话语用 IGNORE_INDEX 掩码。

Args:
 sources (Sequence[str]): 包含多个对话列表的序列，每个对话列表是一个字符串列表。
 tokenizer (transformers.PreTrainedTokenizer): 用于分词的预训练分词器。
